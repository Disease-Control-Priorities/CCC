...
#convert intersectoral PIN back to total pop for years 2023-2030 and scale-up by 1/3 each year 2023-2025
#all.pin.icer<-all.pin.icer%>%
#  mutate(pin = ifelse(group=="Adjusted" & year_id==2023 & Code>5, ((1/3)*(1-Coverage+0.1)+(Coverage-0.1))*(pin/(Coverage)), pin),
#         pin = ifelse(group=="Adjusted" & year_id==2024 & Code>5, ((2/3)*(1-Coverage+0.1)+(Coverage-0.1))*(pin/(Coverage)), pin),
#         pin = ifelse(group=="Adjusted" & year_id>=2025 & Code>5, (pin/(Coverage)), pin)
#         )
...
# All data saved
```{r}
file.name <- paste0("output/bca_costs/All_total_cost_2030_intervention_new",".csv")
write.csv(final_all_intervention, file.name)

file.name <- paste0("output/bca_costs/All_total_cost_2030_year_new",".csv")
write.csv(final_all, file.name)

file.name <- paste0("output/bca_costs/All_ICER_new",".csv") #FOR WILL's CODE
write.csv(final_all_ICER, file.name)

file.name <- paste0("output/bca_costs/All_increment_2030_new",".csv")
write.csv(increment_all_year, file.name)

save(all.pin, all.dalys, all.q30, file = paste0("output/bca_costs/icer_rank_output.Rda"))
