...
project_pop <- function(quality, is, inter, inc, inc.val, inc.type, sel.cse, tas, est.pin, covid.in, sc.covid= NULL){
  # fixed parameters
  ...
  in.df.in    <-  int.df %>% 
    ...
    mutate(`Baseline Coverage`=as.numeric(`Baseline Coverage`),
           inc = ifelse(Code %in% inter, inc, 0)) %>%
  ...
  for(x in years) {
    if (inc.type == "varying"){
      ss = ifelse(i<(2022-year0), 0, i - (2022-year0))
    } else {
      ss = ifelse(i<(2022-year0), 0, i - (2022-year0))
    }
    
    set(cov.ass1, j = x, value = min(0.8, cov.ass1$`Baseline Coverage` + ss*cov.ass1$inc))
    i = i + 1
  }
