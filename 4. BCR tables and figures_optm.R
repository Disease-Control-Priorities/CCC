...
df.cost<-left_join(df, ccc.vsl)%>%
  ...
  mutate(Gross.benefits = val*DALYS.avert,
         Forgone.surplus = ifelse(Code%in%c(5.1,5.2,5.3,5.4), (Gross.benefits*0.00615), 0), #consumer surplus
         Forgone.surplus = ifelse(Code %in% c(5.5,5.6), (Gross.benefits*0.0004), Forgone.surplus)
  )
...
write.csv(df, "Figures/clinical_full_2023_2030_optm_5perc.csv")
...
write.csv(df, "Figures/clinical_full_2023_2030_optm_14perc.csv")
