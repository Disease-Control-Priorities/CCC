...
#' @param tas       : yes/no for intersectorals

project_pop <- function(quality, is, inter, inc.base, inc.val, inc.type, start.year, stop.year, sel.cse, tas, est.pin, covid.in, sc.covid= NULL){
  # fixed parameters
  ...
  if (any(dis.df.in$metric=="disability")){
    ...
    dis.dfb<- dis.df.in %>%
      ...
      mutate(ic = ifelse((start.year-year_id)>0, 0, (year_id-start.year)),
             ...
             `Target (P1)`= ifelse(Pred > inc.val, inc.val, Pred), #this will hold coverage at target after achievement**
    ...
    for(i in 1:n){
      ...
      in.dfb             <- in.df.in %>%
        mutate(ic = ifelse(i < 5, 0, i - 4),
               ...
               `Target (P1)`= ifelse(Pred > inc.val, inc.val, Pred),
               `Target (P1)`= ifelse(`Target (P1)` < `Baseline Coverage`, `Baseline Coverage`, `Target (P1)`),
               `Impact` = quality*((`Effect (I)`*(`Target (P1)`-`Baseline Coverage`))/
                                 (1-(`Effect (I)`*`Baseline Coverage`)))) 
      } else {
