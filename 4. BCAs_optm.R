...
###########################################
load("for_parallel_processing/output2023_optm.Rda")

names<-read.csv("new_inputs/PIN_new.csv", stringsAsFactors = F)%>%
  select(NCD, Intervention)%>%
  rename(Code = NCD)%>% distinct()

df<-read.csv("output/bca_costs/All_increment_2030_optm.csv", stringsAsFactors = F)%>%
...
newdf<-df%>%
  ...
  mutate(DALY.benefit = ifelse(Code%in%c(5.1,5.2,5.3,5.4), DALY.ave*0.14, DALY.ave), #consumer surplus 86%
         DALY.benefit = ifelse(Code %in% c(5.5,5.6), DALY.benefit*0.70, DALY.benefit),  #consumer surplus 30%
...
write.csv(bind_rows(df_code, df_both)%>%arrange(wb2021), "Figures/BCRs_int_optm.csv")
...
write.csv(df_pkg, "Figures/BCRs_pkg_optm.csv")
...
ggsave("Figures/bubbleplot_log_optm.jpeg", height=8, width=10)
...
ggsave("Figures/bubbleplot_optm.jpeg", height=8, width=10)
...
ggsave("Figures/bubbleplot_log_region_optm.jpeg", height=8, width=12)
...
#add intervention names
write.csv(bind_rows(df_code, df_both)%>%arrange(wb2021), "Figures/BCRs_int_optm_5perc.csv")
...
write.csv(df_pkg, "Figures/BCRs_pkg_optm_5perc.csv")
...
ggsave("Figures/bubbleplot_log_optm_5perc.jpeg", height=8, width=10)
...
ggsave("Figures/bubbleplot_optm_5perc.jpeg", height=8, width=10)
...
ggsave("Figures/bubbleplot_log_region_optm_5perc.jpeg", height=8, width=12)
...
#add intervention names
write.csv(bind_rows(df_code, df_both)%>%arrange(wb2021), "Figures/BCRs_int_optm_14perc.csv")
...
write.csv(df_pkg, "Figures/BCRs_pkg_optm_14perc.csv")
...
ggsave("Figures/bubbleplot_log_optm_14perc.jpeg", height=8, width=10)
...
ggsave("Figures/bubbleplot_optm_14perc.jpeg", height=8, width=10)
...
ggsave("Figures/bubbleplot_log_region_optm_14perc.jpeg", height=8, width=12)
