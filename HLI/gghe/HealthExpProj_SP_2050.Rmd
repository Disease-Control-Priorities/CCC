...
### UPDATE DATA ###
ncd_costs_aug <- read.csv("Figures/Total_cost_NCD_forAdrian0830_new.csv") 
...
#refine wpp
wpp_total<-wpp_raw %>%
  dplyr::filter(Variant=="Medium", Time>=2000, Time<=2050) %>%
...
USD2017_defl<-weo %>%
  ...
  rbind(data.frame(year=seq(2025,2050), value=rep(0,26))) %>%
...
core_df1<-ncd_costs_new %>%
  ...
  distinct() %>%
  #DEFINE TIME WINDOW 2000-2050
  slice(rep(1:n(), each=2050-2000+1)) %>%
  group_by(ISO) %>%
  mutate(year=seq(2000, 2050)) %>%
...
#define ceiling values for yearly GDP growth 2025-2050
gdp_limits<-data.frame(year=seq(2023,2050), #start @ 2023,2030
                       gdp_ceiling=c(rep(1.06,3), rep(1.05,25))) 
#might update this window as well
...
gdp_proj_viz1 %>%
  ...
  scale_x_continuous(breaks=c(2010, 2015, 2020, 2025, 2030, 2035, 2040, 2045, 2050)) +
  theme(legend.direction="vertical", legend.position="bottom",
        panel.background = element_blank()) +
...
out<-main_df_gghe_work1%>%
  filter(GGHEDscenario=="own projections")%>%
  select(ISO, Country, year, GGHED_USD2017, GDP_USD2017, PopTotal)%>%
  rename(GGHED = GGHED_USD2017, GDP = GDP_USD2017)

new_regions<-read_excel("../../new_inputs/HS123.xlsx", sheet="indic_2")%>%
  select(ISO3, HSP)%>%
  rename(ISO=ISO3)

out<-left_join(out, new_regions)

write.csv(out, "gghed_data_2050.csv")
##gghed here

out2<-out%>%group_by(HSP, year)%>%
  na.omit()%>%
  summarise(GDP = sum(GDP), GGHED = sum(GGHED), PopTotal=sum(PopTotal))


write.csv(out2, "gghed_data_HSP.csv", row.names = F)
...
#bar chart
ncd_gghed_boxplots1 %>% 
  filter(year %in% c(2020, 2025, 2030, 2035, 2040, 2045, 2050), GGHEDscenario=="own projections") %>%
...
plot1 <- ncd_gghed_boxplots2a %>% 
  filter(year %in% c(2023, 2026, 2030, 2035, 2040, 2045, 2050), GGHEDscenario=="own projections",
