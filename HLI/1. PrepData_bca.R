...
ssb_eff<-read.csv("../new_inputs/paf_sdi.csv", stringsAsFactors = F)%>%
  filter(Risk.factor == "Diet high in sugar-sweetened beverages")

unique(ssb_eff$Cause[ssb_eff$PAF>=0.002])
unique(cause.mx$cause_name)

ssb_eff<-ssb_eff%>%filter(Cause %in% c("Diabetes mellitus","Chronic kidney disease due to diabetes mellitus",
                                       "Hypertensive heart disease", "Oesophageal cancer", "Kidney cancer", 
                                       "Hemorrhagic stroke","Asthma"))%>%
  mutate(Cause = ifelse(Cause == "Diabetes mellitus", "Diabetes mellitus type 1", Cause),
         Cause = ifelse(Cause == "Chronic kidney disease due to diabetes mellitus", "Chronic kidney disease due to diabetes mellitus type 1", Cause),
         Cause = ifelse(Cause == "Oesophageal cancer", "Esophageal cancer", Cause),
         Cause = ifelse(Cause == "Hemorrhagic stroke", "Intracerebral hemorrhage", Cause))

ssb_eff<-bind_rows(ssb_eff, ssb_eff%>%mutate(Cause = ifelse(Cause == "Diabetes mellitus type 1", "Diabetes mellitus type 2", Cause),
                                             Cause = ifelse(Cause == "Chronic kidney disease due to diabetes mellitus type 1", "Chronic kidney disease due to diabetes mellitus type 2", Cause)))%>%
  mutate(`Mortality reduction` = 0.2*PAF)%>%
  rename(cause_name = Cause)%>%
  select(iso3, location_name, cause_name, `Mortality reduction`)%>%
  mutate(Code = 5.7, `Baseline Coverage` = 0, Intervention='SSB',
         agel = 0, ageu = 85, max.cov=1, target.coverage = 1)%>%
  group_by(location_name)%>%mutate(sub_id = row_number())%>%ungroup()

ssb_eff<-na.omit(left_join(ssb_eff, cse_g, by="cause_name")%>%mutate(`Mortality reduction` = `Mortality reduction`*aff_frac)%>%
                    select(-c(aff_frac)))
...
int.df<-rbind(int.df%>%mutate(target.coverage = 1), salt_eff%>%mutate(metric="case fatality"), 
              ...
              aclp_eff%>%mutate(metric="case fatality"), ssb_eff%>%mutate(metric="case fatality"))
...
save.image(file = "PreppedData2050.Rda")
save.image(file = "../for_parallel_processing/PreppedData2050_ssb.Rda")
