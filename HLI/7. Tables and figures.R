...
DA2<-bind_rows(dadt.all.opt%>%select(location_name, Deaths.Avert), 
              dadt.all%>%select(location_name, Deaths.Avert))%>%
  left_join(., WB)%>%group_by(HSP)%>%
  summarise(Cumulative.deaths.averted = sum(Deaths.Avert))%>%
  na.omit()

DALYS2<-bind_rows(dalys.opt%>%select(location_name, DALY.ave), 
                 all.dalys%>%select(location_name, DALY.ave))%>%
  left_join(., WB)%>%group_by(HSP)%>%
  summarise(Cumulative.DALYs.averted = sum(DALY.ave))%>%
  na.omit()

inc.cost2<-left_join(cost2, WB)%>%group_by(HSP)%>%
  summarise(Cumulative.incremental.cost = sum(Incremental.cost, na.rm=T))%>%
  na.omit()
...
plot3<-read.csv("figures/Table1.csv", stringsAsFactors = F)%>%
  select(HSP, Cumulative.incremental.cost, Cumulative.DALYs.averted)%>%
...
ggplot(plot3, aes(x=HSP, y=value, alpha=Scenario, fill=Measure, group=Measure))+
...
ggsave("figures/Figure3.jpeg", height=6, width=9)
