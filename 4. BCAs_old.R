...
#need to update this so that consumer surplus only impacts benefit calcs
newdf<-df%>%
  ...
  mutate(DALY.ave = ifelse(Code%in%c(5.1,5.2,5.3,5.4), DALY.ave*0.10, DALY.ave), #consumer surplus
         DALY.ave = ifelse(Code %in% c(5.5,5.6), DALY.ave*0.50, DALY.ave),
         DALY.ave = ((1-0.05)^(year_id-2022))*DALY.ave,
         sum_increment_all =  ((1-0.05)^(year_id-2022))*sum_increment_all)%>% #health and cost discounting at 5%
  group_by(location_name, Code, `Intervention package`)%>%
  summarise(DALY.ave = sum(DALY.ave, na.rm=T),
            sum_increment_all = sum(sum_increment_all, na.rm=T))


df_code<-left_join(newdf, gdp)%>%
...
write.csv(df_code, "Figures/BCRs_int.csv")

df_code2<-left_join(newdf, gdp)%>%
  mutate(benefit = 1.65*gdp_pc*DALY.ave)%>%
  group_by(Code, location_name, `Intervention package`)%>%
  summarise(`Incremental cost (billions)` = sum(sum_increment_all, na.rm=T)/1e9,
            ` DALY.ave (millions)` = sum(DALY.ave, na.rm=T)/1e6,
            ` benefit (billions)`=sum(benefit, na.rm=T)/1e9,
            BCR =  ` benefit (billions)`/`Incremental cost (billions)`)%>%
...
write.csv(df_code2, "Figures/BCRs_bycountry.csv")

##Appendix table
WB<-read.csv("new_inputs/country_groupings.csv", stringsAsFactors = F)%>%
  select(iso3, wb2021, location_gbd)%>%rename(location_name = location_gbd)

tab<-df_code2%>%left_join(., WB)%>%
  group_by(wb2021, Code, Intervention, `Intervention package`)%>%
  summarise(cost = sum(`Incremental cost (billions)`, na.rm=T),
            benefit = sum(` benefit (billions)`, na.rm=T))%>%
  mutate(BCR = benefit/cost)

#write.csv(tab, "ccc/Figures/appendix_tab_int.csv", row.names = F)

tab2<-tab%>%group_by(`Intervention package`, wb2021)%>%
  summarise(cost = sum(cost),
            benefit = sum(benefit))%>%
  mutate(BCR = benefit/cost)

write.csv(bind_rows(tab,tab2), "Figures/appendix_tab.csv", row.names=F)
...
df_pkg<-left_join(newdf, gdp)%>%
  mutate(benefit = 1.65*gdp_pc*DALY.ave)%>%
  group_by(`Intervention package`)%>%
  summarise(`Incremental cost (billions)` = sum(sum_increment_all, na.rm=T)/1e9,
           ` DALY.ave (millions)` = sum(DALY.ave, na.rm=T)/1e6,
           ` benefit (billions)`=sum(benefit, na.rm=T)/1e9,
            BCR =  ` benefit (billions)`/`Incremental cost (billions)`)%>%
...
#how much from int
DA<-sum(lic$` DALY.ave (millions)`)
int<-sum(lic$` DALY.ave (millions)`[lic$Code>5])

int/DA

lmic<-newdf%>%
  filter(location_name%in%lmics)%>%
  left_join(., gdp)%>%
  mutate(benefit = 1.65*gdp_pc*DALY.ave)%>%
  group_by(Code, `Intervention package`)%>%
  summarise(`Incremental cost (billions)` = sum(sum_increment_all, na.rm=T)/1e9,
...
DA<-sum(lmic$` DALY.ave (millions)`)
int<-sum(lmic$` DALY.ave (millions)`[lmic$Code>5])

int/DA

df_reg<-bind_rows(lic,lmic)%>%
  select(Code, Intervention, BCR, region, `Intervention package`)%>%
  spread(region, BCR)%>%
  mutate(ratio = LMIC/LIC)

mean(df_reg$ratio, na.rm=T)

write.csv(df_reg, "Figures/BCRs_region.csv")

df_reg<-bind_rows(lic,lmic)%>%
  select(Code, Intervention, BCR, region, `Intervention package`, ` DALY.ave (millions)`)
...
ggsave("Figures/bubbleplot_log.jpeg", height=6, width=10)
...
ggsave("Figures/bubbleplot.jpeg", height=6, width=10)
...
ggsave("Figures/bubbleplot_log_region.jpeg", height=6, width=12)
