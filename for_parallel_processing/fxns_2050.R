...
project_pop <- function(quality, is, inter, inc.base, inc.val, inc.type, start.year, stop.year, sel.cse, est.pin, covid.in, sc.covid= NULL){

  # fixed parameters
  n = 32; year0 = 2019; gender_lab = c("Female", "Male"); sex_ratio = 0.5; 
  ...
  in.df.in    <-  int.df %>% 
    ...
    arrange(Code, sub_id) %>% 
    select(location_name, Intervention, Code, sub_id, cause_name, `Mortality reduction`) %>%
    mutate(`Baseline Coverage`=as.numeric(inc.base),
  ...
  dis.df.in<-  int.df %>% 
    filter(location_name == is,
           Code %in% inter) %>% 
    arrange(Code, sub_id) %>% 
    select(location_name, Intervention, Code, sub_id, cause_name, `Mortality reduction`, metric) %>%
    mutate(`Baseline Coverage`=as.numeric(inc.base),
           inc = ifelse(Code %in% inter, (inc.val-`Baseline Coverage`)/(stop.year-start.year), 0)) %>%
    rename(`Effect (I)` = "Mortality reduction") %>%
    distinct()
  ...
  cse.yll     <- int.df %>% filter(Code %in% inter) %>% pull(cause_name) %>% unique() %>% sort()
  
  # Demographic data
  wpp.ina     <- wpp.in %>% 
    filter(location_name == is & year %in% 2019:2050) %>% select(-c(location_name, iso3)) %>%
  ...
  if (any(dis.df.in$metric=="disability")){
    ...
    dis.dfb<- dis.df.in %>%
      ...
      mutate(ic = ifelse((start.year-year_id)>0, 0, (year_id-start.year)),
             Pred = ifelse(year_id>=stop.year, inc.val,`Baseline Coverage` + ic*inc),
    ...
    dft.dis  <- merge(dis.dfb, data.table(age_name = 0:85))%>%
      filter(year_id>=2020)%>%
      distinct()
    ...
    disab.df<-disab.df%>%bind_rows(disab.df%>%mutate(sex="Male"))%>%
      arrange(year_id,cause_name,sex)
  ...
  doproj <- function(s.dp){
    ...
    for(i in 1:n){
      ...
      in.dfb             <- in.df.in %>%
        mutate(ic = ifelse(i < (start.year-year0+2), 0, i - (start.year-year0+1)),
               Pred = ifelse(s.dp==1 && i>=stop.year-year0+2 , inc.val,`Baseline Coverage` + ic*inc*s.dp),
               `Target (P1)`= ifelse(Pred > 1, 1, Pred), #this will hold coverage at 100% after achievement**
               `Target (P1)`= ifelse(`Target (P1)` < `Baseline Coverage`, `Baseline Coverage`, `Target (P1)`),
               `Impact` = quality*((`Effect (I)`*(`Target (P1)`-`Baseline Coverage`))/
                                 (1-(`Effect (I)`*`Baseline Coverage`)))) 
      ...
      if(any(c(5.1,5.2,5.3,5.4,5.5,5.6, 5.7, 5.8) %in% inter)){
        #scale for intersectorals (code>5)
        #scal = ifelse(i < 5, 0, 
        #              ifelse(i == 5, 1/3, 
        #                     ifelse(i == 6, 2/3, 1)))
        #over-ride intersectoral intervention scale-ups#
        in.dfb<- in.dfb%>%left_join(ta_delay%>%filter(ic==i)%>%select(-c(ic)), by=c("cause_name"))%>%
          mutate(Impact = ifelse(Code%in%c(5.2,5.4), Impact * Alcohol,
                                 ifelse(Code%in%c(5.1,5.3), Impact * Smoking, Impact)))%>%select(-c(Alcohol,Smoking))%>%
          mutate(Impact = ifelse(Code%in%inter, Impact, 0))
  ...
  q150 <- q15.est(P0, D0, n)
  q151 <- q15.est(P1, D1, n)
  ...
  q150d <- data.table(t(q150))
  q151d <- data.table(t(q151))
  
  setnames(q300d, paste0("V",1:32), paste0(2019:2050))
  setnames(q301d, paste0("V",1:32), paste0(2019:2050))
  
  setnames(q150d, paste0("V",1:32), paste0(2019:2050))
  setnames(q151d, paste0("V",1:32), paste0(2019:2050))
  ...
  q15df <- left_join(q150d %>% gather(year_id, Baseline),
                     q151d %>% gather(year_id, Adjusted),
                     by = "year_id") %>% 
    mutate(q15.ave = Baseline - Adjusted, location_name = is, 
           n.inters = ninters)
  ...
  DAdt <- data.table(`Under 30`= DA1, `30-69` = DA2, `70+`=DA3,  year_id = 2019:2050) %>%
    gather(age_group, Deaths.Avert, -year_id) %>% filter(year_id > 2019) %>%
    mutate(location_name = is,  n.inters = ninters)
  
  #NCD death envelope (baseline)
  N0p = mx.dis.0 * P0
  N0p = N0p[1:86,] + N0p[87:172,]
  N0pa = apply(N0p[1:30,],2,sum); N0pb = apply(N0p[31:70,], 2, sum); N0pc = apply(N0p[71:86,], 2, sum)
  
  NCDdt <- data.table(`Under 30`= N0pa, `30-69` = N0pb, `70+`=N0pc,  year_id = 2019:2050) %>%
    gather(age_group, NCD.Deaths0, -year_id) %>% filter(year_id > 2019) %>%
    mutate(location_name = is,  n.inters = ninters)
  
  #NCD death envelope (intervention)
  N1p = mx.dis.1 * P1
  N1p = N1p[1:86,] + N1p[87:172,]
  N1pa = apply(N1p[1:30,],2,sum); N1pb = apply(N1p[31:70,], 2, sum); N1pc = apply(N1p[71:86,], 2, sum)
  
  NCDdt_int <- data.table(`Under 30`= N1pa, `30-69` = N1pb, `70+`=N1pc,  year_id = 2019:2050) %>%
    gather(age_group, NCD.Deaths1, -year_id) %>% filter(year_id > 2019) %>%
    mutate(location_name = is,  n.inters = ninters)
  
  #joins
  DAdt<-left_join(DAdt, NCDdt)
  DAdt<-left_join(DAdt, NCDdt_int)
  ...
  if (length(inter) == 0){
    list(ltcause = ltcause, P1 = P1, D1cse = D1cse, P0 = P0, D0cse = D0cse)  
  } else {
    ...
    mx.pin.1 <- b1$mx.pin %>% arrange(cause_name, sex_name, age_name) %>%
      spread(year_id, mx.dis) %>% select(paste0(2020:2050)) %>% as.matrix()
    ...
    if (est.pin == "yes"){
      ...
      setnames(df0, paste0("V",1:31), paste0(2020:2050))
      ...
      setnames(df1, paste0("V",1:31), paste0(2020:2050))
      ...
      for(x in years) {
        if (inc.type == "varying"){
          ss = ifelse(i<(start.year-year0), 0, i - (start.year-year0))
          sts = ifelse(i>=stop.year-year0, 1,0)
        } else {
          ss = ifelse(i<(start.year-year0), 0, i - (start.year-year0))
          sts = ifelse(i>=stop.year-year0, 1,0)
        }
        
        set(cov.ass1, j = x, value = ifelse(sts==0, cov.ass1$`Baseline Coverage` + ss*cov.ass1$inc, inc.val))
        i = i + 1
      }
      ...
      list(q30df = q30df, q15df = q15df, DAdt = DAdt, ltcause = ltcause, P1 = P1, D1cse = D1cse, P0 = P0, D0cse = D0cse,
      ...
      add_lt <- function(pops, dths, sources) {
        ...
        out_df <- data.table(
          year_id = 2019:2050, group = sources,
      ...
      births_df <- data.table(location_name = is, 
                              rbind(data.table(year_id = 2019:2050, group = "Baseline", Female = Bf0, Male = Bm0),
                                    data.table(year_id = 2019:2050, group = "Adjusted", Female = Bf1, Male = Bm1)))
