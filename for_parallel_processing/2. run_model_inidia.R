...
interventions <-  pin.groups %>% filter(calc_ICER=="yes", Code<5.7) %>% pull(Code) %>% unique() %>% sort()
all.locs<-"India"
...
all.dalys <- rbindlist(everything[[2]])
all.q30 <- rbindlist(everything[[3]])
dadt.all <- rbindlist(everything[[4]])

#########################################################################################

save(all.pin, all.dalys, all.q30, dadt.all, file = "output2023_base_india.Rda") #ADAM run1
...
save(all.pin, all.dalys, all.q30, dadt.all, file = "output2023_pesm.Rda") #ADAM run3
